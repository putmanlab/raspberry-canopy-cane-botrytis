...
* Estimated LW - relative humidity 90     *
* *************************************** *;
...
*	%LET results_path=&base_path.\4_results\environ-epi-day-rh90;
*	%LET results_path_img=&base_path.\4_results\environ-epi-day-rh90\html_images;
	...
	** both;
	%LET name_base=environ-epi-day-rh90_; 
	...
	** Step 1-1 **;
		* OBJ: base model;
		...
		proc glimmix data=environ_epi_day plot=residualpanel method=laplace;
			...
			by experiment;
			model lwe_rh90 = treatment / dist=beta link=logit htype=3;
			title 'rcb env epidem-summ day lwe-rh90 B id base model - step 1 most simple model - step 1-1 base';
		run;
	...
	** Step 2-1 **;
		* OBJ: date;
		...
		proc glimmix data=environ_epi_day plot=residualpanel method=laplace;
			...
			by experiment;
			model lwe_rh90 = treatment date / dist=beta link=logit htype=3;
			title 'rcb env epidem-summ day lwe-rh90 B id base model - step 2 time fixed - step 2-1 date';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;
	...
	** Step 2-2 **;
		* OBJ: date_rating;
		...
		proc glimmix data=environ_epi_day plot=residualpanel method=laplace;
			...
			by experiment;
			model lwe_rh90 = treatment date_rating / dist=beta link=logit htype=3;
			title 'rcb env epidem-summ day lwe-rh90 B id base model - step 2 time fixed - step 2-2 date_rating';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;

	** RESULTS:
		* date: significant improvement in fit
		* date_rating: improvement
	...
	** Step 3-1 **;
		* OBJ: treatment*date_rating interaction;
		...
		proc glimmix data=environ_epi_day plot=residualpanel method=laplace;
			...
			by experiment;
			model lwe_rh90 = treatment|date_rating / dist=beta link=logit htype=3;
			title 'rcb env epidem-summ day lwe-rh90 B id base model - step 3 rating date - step 3-1 treatment*date_rating';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;
		
		* RESULTS: slight decline in fit
		* note: following models tried for dpd18, but when used to examine means, lsmeans return non-est errors that are only fixed by removing date, therefore not run for rh90
			* model lwe_rh90 = treatment date_rating*date / dist=beta link=logit htype=3;
			* model lwe_rh90 = treatment|date_rating date_rating*date / dist=beta link=logit htype=3;
	...
	** Step 4-1 **;
		* OBJ: random block (from 2-2);
		...
		proc glimmix data=environ_epi_day plot=residualpanel method=laplace;
			...
			covtest / wald;
			covtest 'glm' glm;
			title 'rcb env epidem-summ day lwe-rh90 B id base model - step 4 random block - step 4-1 date_rating only';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;	
	...
	** Step 1-1 **;
		* OBJ: autoregressive (from 2-2);
		...
		proc glimmix data=environ_epi_day plot=residualpanel method=laplace;
			...
			covtest / wald;
			covtest 'glm' glm;
			title 'rcb env epidem-summ day lwe-rh90 C repeated measures - step 1 date_rating - step 1-1 ar(1)';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;	
	...
	** Step 1-1 **;
		* OBJ: base model (B step 2-2);
		...
		proc glimmix data=environ_epi_day plot=residualpanel;
			...
			by experiment;
			model lwe_rh90 = treatment date_rating / dist=beta link=logit htype=3;
			title 'rcb env epidem-summ day lwe-rh90 D examine - step 1 update settings - step 1-1 base model';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;	
	...
	** Step 2-1 **;
		* OBJ: repeated measures (C step 1-1);
		...
		proc glimmix data=environ_epi_day plot=residualpanel;
			...
			model lwe_rh90 = treatment / dist=beta link=logit htype=3;
			random date_rating / type=ar(1) subject=treatment*block;
			title 'rcb env epidem-summ day lwe-rh90 D examine - step 1 update settings - step 1-1 repeated measures';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;	
