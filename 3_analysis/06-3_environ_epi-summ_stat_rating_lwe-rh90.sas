...
*	%LET results_path=&base_path.\4_results\environ-epi-rating-rh90;
*	%LET results_path_img=&base_path.\4_results\environ-epi-rating-rh90\html_images;
	...
	** both;
	%LET name_base=environ-epi-rating-rh90_; 
...
*			title 'rcb env epidem-summ day lwe-dpd18 B id base model - step 3 rating date - step 3-1';
*		run;
...
*** Step 4 ***;
	...
	** Step 4-2 **;
		* OBJ: random block (from 2-1);
		* see Appendix B for code		
		* RESULTS: block cov param estimates missing 
			* B cov test < 0.05
	...
	** RESULTS: cov test p-values > 0.05 (except 4-2 B, 4-3 B)
...
*** Step 1 ***;
	...
	** RESULTS: est G matrix error for A, B
	...
	** Step 1-1 **;
		* OBJ: base model (B step 3-1);
		...
		proc glimmix data=environ_epi_rating plot=residualpanel;
			...
			by experiment;
			model lwe_rh90 = treatment|date_rating / dist=beta link=logit htype=3;
			title 'rcb env epidem-summ day lwe-dpd18 D update settings - step 1 update settings - step 1-1 base model';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;	
	...
	** Step 1-1 **;
		* OBJ: base model (B step 3-1);
		...
		proc glimmix data=environ_epi_rating_C plot=residualpanel;
			...
			title 'rcb env epidem-summ day lwe-dpd18 E examine - step 1 means - step 1-1';
		run;
		%output_html_files_off(); * turn off redirecting html, log output to files;	

		* RESULTS: blade ab, control bc, manual a, twine c
	
	** CONCLUSION: analysis complete
		* same results as dpd18
...
*			title 'rcb env epidem-summ day lwe-dpd18 B id base model - step 1 most simple model - step 1-1';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: 
...
*			by experiment;
*			model lwe_rh90 = treatment date_rating / dist=beta link=logit htype=3;
*			title 'rcb env epidem-summ day lwe-dpd18 B id base model - step 2 time fixed - step 2-1';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*	** RESULTS: significant improvement in fit
...
*			covtest / wald;
*			covtest 'glm' glm;
*			title 'rcb env epidem-summ day lwe-dpd18 B id base model - step 4 random block - step 4-1 no block (1-1)';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;	
*
*	** Step 4-2 **;
...
*			covtest / wald;
*			covtest 'glm' glm;
*			title 'rcb env epidem-summ day lwe-dpd18 B id base model - step 4 random block - step 4-2 separate (2-1)';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;	
*		
*		* RESULTS: block cov param estimates missing 
...
*			covtest / wald;
*			covtest 'glm' glm;
*			title 'rcb env epidem-summ day lwe-dpd18 B id base model - step 4 random block - step 4-3 interaction (3-1)';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;	
*		
*		* RESULTS: block cov param estimates missing
