...
*	%LET results_path=&base_path.\4_results\environ-hour-temp;
*	%LET results_path_img=&base_path.\4_results\environ-hour-temp\html_images;
	...
	** both;
	%LET name_base=environ-hour-temp_; 
...
*** Step 5 ***;
	...
	** Step 5-2 **;
		...
		* note: not run (was stopped after 2 hr for rel. humidity)
	...
	** Step 6-3 **;
		* OBJ: add rating date to b interaction;
		...
		proc glimmix data=environ_hr plot=residualpanel method=laplace;
			...
			by experiment;
			model temp_avg = treatment|date_rating|period date_rating*week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
			title 'rcb env hour temp B id base model - step 6 rating date - 6-3 add to a,b interaction';
		run;
...
*			by experiment;
*			model temp_avg = treatment|period / dist=normal link=identity htype=3 ddfm=bw;
*			random week*day_of_week*period*hour / type=cs subject=treatment*block;
*			covtest / wald;
*			covtest 'glm' glm;
*			title 'rcb env hour temp C repeated measure simple - step 1 repeated measures - 1-1 compound symmetry';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: 
...
*			covtest / wald;
*			covtest 'glm' glm;
*			title 'rcb env hour temp B repeated measure simple - step 6 repeated measures - 1-2 autoregressive';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
...
*			by experiment;
*			model temp_avg = treatment / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 1 base (no repeated) - 1-1 base';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
...
*			by experiment;
*			model temp_avg = treatment datetime / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 2 datetime (no repeated) - 2-1 datetime';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: significant improvement vs step 1-1 (see spreadsheet)
...
*			by experiment;
*			model temp_avg = treatment week*day_of_week*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 2 datetime (no repeated) - 2-2 week, day of week, hour';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
...
*			class experiment treatment week day_of_week period hour;
*			by experiment;
*			model temp_avg = treatment week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 3 test period - 3-1 add period in b interaction';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: identical to step 2-2
...
*			class experiment treatment week day_of_week period hour;
*			by experiment;
*			model temp_avg = treatment|period week*day_of_week*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 3 test period - 3-2 add period in a interaction';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: very slight improvement vs Step 3-1, but period results missing
...
*			class experiment treatment week day_of_week period hour;
*			by experiment;
*			model temp_avg = treatment|period week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 3 test period - 3-3 add period in a, b interaction';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: same as 3-2 except period result present
...
*			class experiment treatment week day_of_week period hour;
*			by experiment;
*			model temp_avg = treatment|period|hour week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 4 test hour - 4-1 add hour to a interaction';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
...
*			by experiment;
*			model temp_avg = treatment|period week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 5 random block - 5-1 add block to class statement';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
...
*			class experiment treatment date_rating week day_of_week period hour;
*			by experiment;
*			model temp_avg = treatment|date_rating|period week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 6 rating date - 6-1 add to a interaction';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
*
*		* RESULTS: date_rating fixed effect estimates missing
...
*			class experiment treatment date_rating week day_of_week period hour;
*			by experiment;
*			model temp_avg = treatment|period date_rating*week*day_of_week*period*hour / dist=normal link=identity htype=3 ddfm=bw;
*			title 'rcb env hour temp B id base model - step 6 rating date - 6-2 add to b interaction';
*		run;
*		%output_html_files_off(); * turn off redirecting html, log output to files;
